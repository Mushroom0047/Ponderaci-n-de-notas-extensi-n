...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //* Eliminar puntos de input
    document.querySelectorAll('input[type="number"]').forEach(input => {
        input.addEventListener('input', function() {
            // Elimina los puntos del valor ingresado
...
function calcularPromedio() {    
    ...
    if (sumaDePonderaciones !== 100) {
        ...
        document.querySelector('.label-promedio').textContent = `${promedioPonderado.toFixed(2)}`;
...
function alertMessage(message) {
    ...
    if (message) {
        ...
        alertContainer.classList.add('flex', 'items-center', 'p-2', 'gap-2');
        ...
        svgIcon.classList.add('w-6', 'h-6');  // Ajusta la clase para el tamaño del icono
        ...
        // Agregar el SVG y el párrafo al contenedor
        alertContainer.appendChild(svgIcon);
        alertContainer.appendChild(newP);

        // Agregar el contenedor al div con id "alerts"
        divAlerts.appendChild(alertContainer);
    }
}
...
function CreateNewInput(qtyItemsToCreate = 1) {
    ...
    while (qtyItemsToCreate > 0) {
        ...
        newRow.innerHTML = `
            ...
            <div class=" flex text-center justify-center items-end">
